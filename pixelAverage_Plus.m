%% Pixel map averaging in the '+' fashion 
% 
...
%    z = eye(3,3);
%    u1=2.2; v1= 2.4;
%    ans = pixelAverage_Plus(x,y,z,u1,v1)
%    u 
%    v

function pxl_avg_coords = pixelAverage_Plus(x,y,z,u1,v1)
    ...
    pxl_avg_coords(1) = (1/5)*(x(u_cen,v_cen)+x(u_p,v_cen)+x(u_cen,v_p)+x(u_cen,v_m)+x(u_m,v_cen));
    pxl_avg_coords(2) = (1/5)*(y(u_cen,v_cen)+y(u_p,v_cen)+y(u_cen,v_p)+y(u_cen,v_m)+y(u_m,v_cen));
    pxl_avg_coords(3) = (1/5)*(z(u_cen,v_cen)+z(u_p,v_cen)+z(u_cen,v_p)+z(u_cen,v_m)+z(u_m,v_cen));
end
